...
def run_hook(
    ...
    # Set environment variables for Python scripts to locate the project
    # These are read by scripts/core/_env.py
    if project_root:
        run_env["CLAUDE_PROJECT_DIR"] = str(project_root)
        # For scripts in opc/scripts/, set CLAUDE_OPC_DIR
        opc_dir = project_root / "opc" if (project_root / "opc").exists() else project_root
        run_env["CLAUDE_OPC_DIR"] = str(opc_dir)
        # Set PYTHONPATH so uv run subprocess can find modules
        current_pythonpath = run_env.get("PYTHONPATH", "")
        if current_pythonpath:
            run_env["PYTHONPATH"] = f"{opc_dir}:{current_pythonpath}"
        else:
            run_env["PYTHONPATH"] = str(opc_dir)
